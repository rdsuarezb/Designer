...
function clickEnAdd(e){
    ...
    var cuadroEntidad=this.parent().parent().select(".cuadroEntidad");
    ...
    cuadroEntidad.animate({height:cuadroEntidad.getBBox().height+cuadroElemento.getBBox().height} ,300);
...
function colocarInput(e){
   ...
   //alert(this.parent().selectAll("rect")[1]);
    ...
    if(this.attr("class")=="cuadroNombreEntidad"){
        ...
        input.style.width=this.parent().select(".cuadroNombreEntidad").getBBox().width+"px";
        input.style.height=this.parent().select(".cuadroNombreEntidad").getBBox().height+"px";
        if(this.parent().matrix!=null){
        input.style.left=this.parent().select(".cuadroNombreEntidad").getBBox().x+this.parent().matrix.e+"px";
        input.style.top=this.parent().selectAll(".cuadroNombreEntidad").getBBox().y+this.parent().matrix.f+"px";
        }
        else{
            //alert(this.parent().selectAll("rect")[1].getBBox().x);
        input.style.left=this.parent().select(".cuadroNombreEntidad").getBBox().x+"px";
        input.style.top=this.parent().select(".cuadroNombreEntidad").getBBox().y+"px";
        }
...
var cuadroEntidad=img.rect(posx,posy,ancho,alto,bordeado,bordeado);
...
circuloRelacion.drag(function(dx,dy){
            //var listaEntidades=img.selectAll(".entidad");
            var i;
            circle.attr({cx:+x1+dx,cy:+y1+dy});
            line.attr({x1:+x1+dx,y1:+y1+dy});

           // console.log(circle.getBBox().cx+" "+circle.getBBox().cy);
             
            /*
    
            for(i=0;i<listaEntidades.length;i++){
               console.log(listaEntidades[i].attr("id"));
                var cuadroEntidad=listaEntidades[i].select(".cuadroEntidad");
                var cuadroEntidadX=cuadroEntidad.getBBox().x+listaEntidades[i].transform().localMatrix.e;
                var cuadroEntidadX2=cuadroEntidad.getBBox().x2+listaEntidades[i].transform().localMatrix.e;
                var cuadroEntidadY2=cuadroEntidad.getBBox().y2+listaEntidades[i].transform().localMatrix.f;
                var cuadroEntidadY=cuadroEntidad.getBBox().y+listaEntidades[i].transform().localMatrix.f;
               //console.log(listaEntidades[i].transform().localMatrix.e+" "+listaEntidades[i].transform().localMatrix.f);
                if(listaEntidades[i]!=this.parent()){
                     if(circle.getBBox().cy<cuadroEntidadY2 && circle.getBBox().cy>cuadroEntidadY && circle.getBBox().cx<cuadroEntidadX2 && circle.getBBox().cx>cuadroEntidadX){
                        console.log("dentro de rango");
                         listaEntidades[i].transform("s1,1,1.5");
                     }
                }
...
grupoEntidad.attr({
   id:"E"+contEntidad ,class:"entidad"
});
...
cuadroEntidad.attr({
    class:"cuadroEntidad", fill:"transparent", stroke:"black" 
});
...
grupoEntidad.add(cuadroEntidad,cuadroElementos,cuadroCabeceraEntidad,textoNombreEntidad,cuadroNombreEntidad,cuadroConfig,cuadroMin,grupoAdd,grupoElemento,circuloRelacion);
...
grupoEntidad.mouseover(cursorSobreGrupoEntidad);
//alert(grupoEntidad.selectAll("rect")[1]);
    
}

function cursorSobreGrupoEntidad(e){
    if(relacionando!=false){
...
function cursorUpSvg(e){
   alert(e.target.getAttribute("class"));
}

function inicializar(){
    var img=Snap("#imagen");
    //img.mouseup(cursorUpSvg)
